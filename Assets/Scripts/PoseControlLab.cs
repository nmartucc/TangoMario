...
public class PoseControlLab : MonoBehaviour , ITangoPose {
	...
	public void OnTangoPoseAvailable(Tango.TangoPoseData pose)
	{
		    ...
		    pose.framePair.targetFrame == TangoEnums.TangoCoordinateFrameType.TANGO_COORDINATE_FRAME_DEVICE)
		{
			...
			else // if the current pose is not valid we set the pose to identity
			{
				m_tangoPosition = Vector3.zero;
				m_tangoRotation = Quaternion.identity;
				//Debug.Log (m_tangoPosition +" "+ m_tangoRotation);
			}
		}
	}
	...
	void Update()
	{
		...
		if (Input.GetKeyDown ("space")) {
			net.SendVector3 (m_tangoPosition);
			net.SendQuaternion (m_tangoRotation);
		}*/
		/*
		foreach (Touch touch in Input.touches) {
			DebugConsole.Log(touch.ToString());
			net.SendVector3 (m_tangoPosition);
			DebugConsole.Log("Sending vector");
			net.SendQuaternion (m_tangoRotation);
			DebugConsole.Log("Sending quaternion");
		}
		 */
